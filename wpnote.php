...
class WPNote_Plugin {
    ...
    private function generate_md2card_cover($post_id, $title, $params = array()) {
        ...
        $remote_url = $body['images'][0]['url'];
        
        // 下载图片到本地并转换为webp
        $local_result = $this->download_and_save_cover($remote_url, $post_id, $title);
        ...
        return array(
            'image' => $local_result['url'],
            'md2card_theme' => $theme,
            'image_local' => true,
            'attachment_id' => $local_result['attachment_id'],
        );
    }
    
    /**
     * 下载远程图片到本地，并转换为webp格式
     * 
     * @param string $remote_url 远程图片URL
     * @param int $post_id 关联的文章ID
     * @param string $title 用于生成文件名的标题
     * @return array|WP_Error 成功返回 ['url'=>本地URL, 'attachment_id'=>附件ID]，失败返回WP_Error
     */
    private function download_and_save_cover($remote_url, $post_id, $title = '') {
        // 下载远程图片
        ...
        if (empty($image_data)) {
            return new WP_Error('empty_image', '下载的图片内容为空');
        }
        ...
        // 生成文件名
        $filename = 'wpnote-cover-' . $post_id . '-' . substr(md5($remote_url . time()), 0, 8);
        ...
        // 保存到WordPress上传目录
        $upload = wp_upload_bits($filename . '.' . $final_ext, null, $image_data);
        
        if ($upload['error']) {
            return new WP_Error('save_failed', '图片保存失败: ' . $upload['error']);
        }
        
        // 创建附件
        $attachment = array(
            'post_mime_type' => 'image/webp',
            'post_title' => sanitize_text_field($title ?: 'WPNote封面'),
            'post_content' => '',
            'post_status' => 'inherit',
            'post_parent' => $post_id,
        );
        ...
        if (is_wp_error($attachment_id)) {
            return new WP_Error('attachment_failed', '创建附件失败');
        }
     ...
     * 将图片数据转换为webp格式
     * 
     * @param string $image_data 原始图片二进制数据
     * @param string $source_ext 源格式 (png/jpg/gif)
     * @return string|WP_Error 转换后的webp数据或错误
     */
    private function convert_to_webp($image_data, $source_ext) {
        // 检查GD库是否支持webp
        if (!function_exists('imagewebp') || !function_exists('imagecreatefromstring')) {
            return new WP_Error('gd_unsupported', 'GD库不支持webp转换');
        }
        
        // 从字符串创建图像资源
        $image = @imagecreatefromstring($image_data);
        if ($image === false) {
            return new WP_Error('image_create_failed', '无法创建图像资源');
        }
        
        // 转换为webp
        ob_start();
        $quality = get_option('wpnote_webp_quality', 85); // webp质量，默认85
        $success = imagewebp($image, $quality);
        $webp_data = ob_get_clean();
        
        // 释放资源
        imagedestroy($image);
        
        if (!$success || empty($webp_data)) {
            return new WP_Error('webp_conversion_failed', 'webp转换失败');
        }
    ...
    public function ajax_generate_cover() {
        ...
        wp_send_json_success(array(
            'message' => '封面生成成功',
            'image_url' => $final_url,
            'local_saved' => !is_wp_error($local_result),
        ));
    }
    ...
    public function settings_page() {
        ...
        // WebP图片设置
        $convert_webp = get_option('wpnote_convert_webp', true);
        $webp_quality = get_option('wpnote_webp_quality', 85);
        echo '<div class="wpnote-card2"><h2>🖼️ 图片本地化设置</h2>';
        echo '<div style="margin-bottom:16px;">';
        echo '<label style="display:flex;align-items:center;gap:8px;cursor:pointer;">';
        echo '<input type="checkbox" name="wpnote_convert_webp" value="1" ' . checked($convert_webp, true, false) . ' style="width:18px;height:18px;">';
        echo '<span style="font-size:14px;"><strong>自动转换为WebP格式</strong>（节省磁盘空间，体积减少30%-80%）</span>';
        echo '</label>';
        echo '</div>';
        echo '<div style="display:flex;align-items:center;gap:12px;">';
        echo '<label style="font-size:14px;">WebP质量：</label>';
        echo '<input type="range" name="wpnote_webp_quality" min="60" max="100" value="' . esc_attr($webp_quality) . '" style="flex:1;">';
        echo '<span style="font-size:14px;font-weight:600;min-width:40px;">' . esc_html($webp_quality) . '%</span>';
        echo '</div>';
